...
public class RectangularTessellator extends WWObjectImpl implements Tessellator
{
    ...
    // seaglassfoundry.com: event-driven invalidation -- only rebuild when globe state (elevation data) changes,
    // instead of forcing a rebuild every 2 seconds regardless of whether anything changed.
    ...
    {
        return computeLocations(tile, new ArrayList<>());
    }

    // seaglassfoundry.com: overload that reuses an existing list to reduce allocation churn
    protected ArrayList<LatLon> computeLocations(RectTile tile, ArrayList<LatLon> latlons)
    {
        int density = tile.density;
        int numVertices = (density + 3) * (density + 3);
        ...
        latlons.clear();
        latlons.ensureCapacity(numVertices);
        for (int j = 0; j <= density + 2; j++)
        {
            Angle lon = lonMin;
            for (int i = 0; i <= density + 2; i++)
...
//        ArrayList<Vec4> workingStorage = new ArrayList<>();
...
//        ArrayList<ArrayList<Vec4>> normalLists = new ArrayList<>(numVertices);
