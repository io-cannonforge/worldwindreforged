...
public class Sector implements Cacheable, Comparable<Sector>, Iterable<LatLon> {
    ...
    public static boolean isSector(Iterable<? extends LatLon> corners) {
        ...
        if (!latlons[0].getLatitude().equals(latlons[1].getLatitude()) || !latlons[2].getLatitude().equals(latlons[3].getLatitude()) || !latlons[0].getLongitude().equals(latlons[3].getLongitude()) || !latlons[1].getLongitude().equals(latlons[2].getLongitude())) {
            return false;
        }
    ...
    public boolean intersectsSegment(LatLon begin, LatLon end) {
        ...
        if ((Math.abs(diff.x) > (boxExtentX + segmentExtent * Math.abs(segmentDirection.x))) || (Math.abs(diff.y) > (boxExtentY + segmentExtent * Math.abs(segmentDirection.y)))) {
            return false;
        }
	...
	public Iterator<LatLon> iterator() {
        return new Iterator<>() {
			...
			public LatLon next() {
                ...
                LatLon p = switch (this.position) {
                    case 0 -> new LatLon(Sector.this.getMinLatitude(), Sector.this.getMinLongitude());
                    case 1 -> new LatLon(Sector.this.getMinLatitude(), Sector.this.getMaxLongitude());
                    case 2 -> new LatLon(Sector.this.getMaxLatitude(), Sector.this.getMaxLongitude());
                    default -> new LatLon(Sector.this.getMaxLatitude(), Sector.this.getMinLongitude());
                };
                ++this.position;
    ...
    public boolean equals(Object o) {
        ...
        //noinspection RedundantIfStatement
        if (!maxLatitude.equals(sector.maxLatitude) || !maxLongitude.equals(sector.maxLongitude) || !minLatitude.equals(sector.minLatitude) || !minLongitude.equals(sector.minLongitude)) {
            return false;
        }
