...
public abstract class AbstractSceneController extends WWObjectImpl implements SceneController
{
    ...
    // seaglassfoundry.com: min/max frame time tracking for performance analysis
    protected double minFrameTime = Double.MAX_VALUE;
    protected double maxFrameTime = 0;
    protected long frameTimeWindowStart = System.currentTimeMillis();
    protected static final long FRAME_TIME_WINDOW_MS = 2000;
    /**
    ...
    //protected Map<String, GroupingFilter> groupingFilters = new HashMap<>();
    protected boolean deferOrderedRendering;
    // seaglassfoundry.com: demand-driven pick pass -- only run full pick when mouse interaction occurs
    protected boolean pickDirty = true;
    protected long lastPickViewStateID = -1;
...
//    }
    ...
    public int repaint()
    {
        ...
        // seaglassfoundry.com: exponential moving average FPS for responsive, per-frame updates
        double instantFps = (this.frameTime > 0) ? 1000.0 / this.frameTime : 0;
        this.framesPerSecond = 0.1 * instantFps + 0.9 * this.framesPerSecond;

        // seaglassfoundry.com: track min/max frame time over a rolling window
        this.minFrameTime = Math.min(this.minFrameTime, this.frameTime);
        this.maxFrameTime = Math.max(this.maxFrameTime, this.frameTime);
        if (time - this.frameTimeWindowStart > FRAME_TIME_WINDOW_MS)
        {
            this.frameTimeWindowStart = time;
        ...
        this.dc.setPerFrameStatistic("gov.nasa.worldwind.perfstat.MinFrameTime",
            "Min Frame Time (ms)", (int) this.minFrameTime);
        this.dc.setPerFrameStatistic("gov.nasa.worldwind.perfstat.MaxFrameTime",
...
//        this.resetGroupingFilters();
    ...
    /**
     * seaglassfoundry.com: determines whether the full pick pass is needed this frame. Returns true only when
     * there is an active pick point or pick rectangle requiring color-coded rendering. Viewport center position
     * is resolved via lightweight ray-intersection when the full pick is skipped.
     */
    protected boolean isPickNeeded(DrawContext dc)
    {
    ...
    /**
     * seaglassfoundry.com: called after a pick pass completes to update tracking state.
     */
    ...
    /**
     * seaglassfoundry.com: resolves viewport center position using CPU ray-intersection against terrain geometry,
     * avoiding the expensive color-coded pick rendering pass when no mouse interaction is active.
     */
    protected void resolveViewportCenterPosition(DrawContext dc)
    {
        ...
        {
            Vec4 point = intersections[0].getIntersectionPoint();
            Position pos = dc.getGlobe().computePositionFromPoint(point);
            dc.setViewportCenterPosition(pos);
        }
    }
