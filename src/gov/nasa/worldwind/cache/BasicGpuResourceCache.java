...
public class BasicGpuResourceCache implements GpuResourceCache
{
    ...
    protected void onEntryRemoved(Object key, Object clientObject)
    {
        ...
        else if (entry.resourceType == TEXTURE_IDS)
        {
            int[] ids = (int[]) entry.resource;
            gl.glDeleteTextures(ids.length, ids, 0);
        }
        // seaglassfoundry.com: VAO cleanup on cache eviction
        else if (entry.resourceType == VAO_IDS)
        {
            int[] ids = (int[]) entry.resource;
            gl.glDeleteVertexArrays(ids.length, ids, 0);
        }
    }
