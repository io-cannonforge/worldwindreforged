...
//                throw new WWRuntimeException( message );
    ...
    protected Dataset createMaskDataset(int width, int height, Sector sector)
    {
        ...
        Dataset ds = drvMem.Create("roi-mask", width, height, 1, gdalconstConstants.GDT_UInt32);
        Band band = ds.GetRasterBand(1);
        band.SetColorInterpretation(gdalconstConstants.GCI_AlphaBand);
...
//            return this.dsVRT;
        ...
        int bestOverviewIdx, int srcWidth, int srcHeight)
    {
        ...
        gt[GDAL.GT_1_PIXEL_WIDTH] = Math.abs((cropArea.getMaxX() - cropArea.getMinX()) / reqWidth);
        gt[GDAL.GT_5_PIXEL_HEIGHT] = -Math.abs((cropArea.getMaxY() - cropArea.getMinY()) / reqHeight);
        ...
        for (int i = 0; i < bandCount; i++)
        {
            ...
            if (colorInt == gdalconstConstants.GCI_PaletteIndex)
            {
    ...
    protected Dataset createCompatibleDataset(int width, int height, Sector sector, AVList destParams)
    {
        ...
        int[] bandColorInt = new int[] {gdalconstConstants.GCI_RedBand, gdalconstConstants.GCI_GreenBand,
            gdalconstConstants.GCI_BlueBand, gdalconstConstants.GCI_AlphaBand};
        ...
        {
            destNumOfBands = 1;
            bandColorInt = new int[] {gdalconstConstants.GCI_GrayIndex};
        }
        else if (AVKey.IMAGE.equals(pixelFormat) && AVKey.GRAYSCALE.equals(colorFormat))
        {
            bandColorInt = new int[] {gdalconstConstants.GCI_GrayIndex, gdalconstConstants.GCI_AlphaBand};
            destNumOfBands = 2; // Y + alpha
        }
        else if (AVKey.IMAGE.equals(pixelFormat) && AVKey.COLOR.equals(colorFormat))
        {
            bandColorInt = new int[] {
                gdalconstConstants.GCI_RedBand, gdalconstConstants.GCI_GreenBand, gdalconstConstants.GCI_BlueBand, gdalconstConstants.GCI_AlphaBand};
            ...
            else
            {
                ...
                bandColorInt = new int[] {gdalconstConstants.GCI_PaletteIndex};
            }
        }
...
//        Double nodata = this.calcNoDataForDestinationRaster(destParams);
        ...
        for (int i = 0; i < destNumOfBands; i++)
        {
            ...
            int colorInt = gdalconstConstants.GCI_Undefined;

            if (null != srcBand)
            {
                colorInt = srcBand.GetColorInterpretation();

                if (colorInt == gdalconstConstants.GCI_Undefined)
                {
            ...
            if (colorInt == gdalconstConstants.GCI_AlphaBand)
            {
                band.Fill(GDALUtils.ALPHA_MASK);
            }
...
//            SpatialReference t_srs = new SpatialReference(t_srs_wkt);
                ...
                if (this.srs != null)
                {
                    ...
                    gdal.ReprojectImage(srcDS, destDS, s_srs_wkt, t_srs_wkt, gdalconstConstants.GRA_Bilinear);
                    projTime = System.currentTimeMillis() - start;

                    start = System.currentTimeMillis();
                    if (null != maskDS)
                    {
                        gdal.ReprojectImage(srcDS, maskDS, s_srs_wkt, t_srs_wkt, gdalconstConstants.GRA_NearestNeighbour);
                    }
