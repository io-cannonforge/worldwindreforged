...
public abstract class AbstractTacticalSymbol extends WWObjectImpl implements TacticalSymbol, Movable, Draggable
{
    protected static class IconSource
    {
        ...
        public boolean equals(Object o)
        {
            ...
            if ((this.retriever != null ? !this.retriever.equals(that.retriever)
                : that.retriever != null) || (this.symbolId != null ? !this.symbolId.equals(that.symbolId) : that.symbolId != null))
                return false;
    ...
    protected void makeOrderedRenderable(DrawContext dc)
    {
        ...
        if (dc.getFrameTimeStamp() != this.frameNumber || dc.isContinuous2DGlobe())
        {
            ...
            // not yet visible.
            if ((!dc.is2DGlobe() && osym.eyeDistance > horizon) || (osym.screenRect == null && !this.intersectsFrustum(dc, osym)))
                return;
    ...
    protected boolean mustLayout(IconSource iconSource, AVList modifiers)
    {
        ...
        if (this.unresolvedGlyph || this.staticScreenRect == null || this.staticLayoutRect == null || !this.activeModifiers.getEntries().equals(modifiers.getEntries()))
            return true;
    ...
    protected void removeDeadModifiers(long now)
    {
        ...
        for (var entry : this.glyphMap.entrySet())
        {
    ...
    protected void drawBatched(DrawContext dc, OrderedSymbol firstSymbol)
    {
        ...
        else if (this.isEnableBatchPicking())
        {
            ...
            {
                OrderedSymbol ts = (OrderedSymbol) nextItem;
                if (!ts.isEnableBatchRendering() || !ts.isEnableBatchPicking() || (ts.getPickLayer() != firstSymbol.getPickLayer())) // batch pick only within a single layer
                    break;
    ...
    protected void beginDrawing(DrawContext dc, int attrMask)
    {
        ...
        if (dc.isPickingMode())
        {
            ...
            gl.glTexEnvi(GL2ES1.GL_TEXTURE_ENV, GL2ES1.GL_SRC0_RGB, GL2ES1.GL_PREVIOUS);
            gl.glTexEnvi(GL2ES1.GL_TEXTURE_ENV, GL2ES1.GL_COMBINE_RGB, GL.GL_REPLACE);
    ...
    protected void endDrawing(DrawContext dc)
    {
        ...
        if (dc.isPickingMode())
        {
            gl.glTexEnvi(GL2ES1.GL_TEXTURE_ENV, GL2ES1.GL_TEXTURE_ENV_MODE, OGLUtil.DEFAULT_TEX_ENV_MODE);
            gl.glTexEnvi(GL2ES1.GL_TEXTURE_ENV, GL2ES1.GL_SRC0_RGB, OGLUtil.DEFAULT_SRC0_RGB);
            gl.glTexEnvi(GL2ES1.GL_TEXTURE_ENV, GL2ES1.GL_COMBINE_RGB, OGLUtil.DEFAULT_COMBINE_RGB);
        }
    ...
    {
        if (this.activeIconTexture == null || this.iconRect == null || !this.activeIconTexture.bind(dc))
            return;
    ...
    {
        if (this.glyphAtlas == null || this.currentGlyphs.isEmpty() || !this.glyphAtlas.bind(dc))
            return;
