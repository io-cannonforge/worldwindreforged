...
public class TerrainProfileLayer extends AbstractLayer implements PositionListener, SelectListener {
    ...
    public int getPathType() {
        return switch (pathType) {
            case AVKey.LINEAR -> Polyline.LINEAR;
            case AVKey.RHUMB_LINE -> Polyline.RHUMB_LINE;
            case AVKey.GREAT_CIRCLE -> Polyline.GREAT_CIRCLE;
            default -> {
    ...
    public void setPathType(int pathType) {
        this.pathType = switch (pathType) {
            case Polyline.LINEAR -> AVKey.LINEAR;
            case Polyline.RHUMB_LINE -> AVKey.RHUMB_LINE;
            case Polyline.GREAT_CIRCLE -> AVKey.GREAT_CIRCLE;
            default -> {
                String message = Logging.getMessage("generic.ArgumentOutOfRange");
                Logging.logger().severe(message);
                throw new IllegalArgumentException(message);
...
//            this.expirySupport.restart(dc);
    ...
    protected double computeScale(java.awt.Rectangle viewport) {
        return switch (this.resizeBehavior) {
            case AVKey.RESIZE_SHRINK_ONLY -> Math.min(1d, (this.toViewportScale) * viewport.width / this.size.width);
            case AVKey.RESIZE_STRETCH -> (this.toViewportScale) * viewport.width / this.size.width;
            case AVKey.RESIZE_KEEP_FIXED_SIZE -> 1d;
            default -> 1d;
        };
    }
...
//            this.wwd.getModel().getGlobe().getElevationModel().addPropertyChangeListener(this);
    ...
    protected void computeProfile(DrawContext dc) {
        ...
        try {
            ...
            Position groundPos = switch (this.follow) {
                case FOLLOW_VIEW -> this.computeViewCenterPosition(dc);
                case FOLLOW_CURSOR -> this.computeCursorPosition(dc);
                case FOLLOW_EYE -> view.getEyePosition();
                case FOLLOW_OBJECT -> this.objectPosition;
                default -> null;
            };
    ...
    protected void computeMirroredPositions(LatLon centerLatLon) {
        ...
        for (i = 0; i < this.samples; i++) {
            ...
            if (!this.follow.equals(FOLLOW_NONE)) {
                ...
                if (i > (this.samples - 1) / 2f) {
                    //azimuth = view.getHeading().subtract(Angle.NEG90).radians;
                    azimuth += Math.PI;
                }
                double distance = Math.abs((i - ((this.samples - 1) / 2d)) * step);
