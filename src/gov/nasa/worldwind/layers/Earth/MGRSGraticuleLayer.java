...
public class MGRSGraticuleLayer extends UTMBaseGraticuleLayer
{
    ...
    {
        if ((lineStyle == null) || (graticuleType == null))
        {
            String message = Logging.getMessage("nullValue.StringIsNull");
            Logging.logger().severe(message);
            throw new IllegalArgumentException(message);
	...
	protected String getTypeFor(int resolution)
    {
        ...
        {
            case 100000 -> GRATICULE_100000M; // 100,000 meters
            case 10000 -> GRATICULE_10000M;   // 10,000 meters
            case 1000 -> GRATICULE_1000M;     // 1000 meters
            case 100 -> GRATICULE_100M;       // 100 meters
            case 10 -> GRATICULE_10M;         // 10 meters
            case 1 -> GRATICULE_1M;           // 1 meter
            default -> null;
        };
    }
    ...
    private class GridZone
    {
        ...
        public void selectRenderables(DrawContext dc, Sector vs, MGRSGraticuleLayer layer)
        {
            ...
            for (GridElement ge : this.gridElements)
            {
                ...
                {
                    if ((ge.type.equals(GridElement.TYPE_LINE_NORTH) && isNorthNeighborInView(this, dc)) || (ge.type.equals(GridElement.TYPE_LINE_EAST) && isEastNeighborInView(this, dc)))
                        continue;
