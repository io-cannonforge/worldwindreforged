...
public class UTMBaseGraticuleLayer extends AbstractGraticuleLayer
{
	...
	protected void selectRenderables(DrawContext dc)
    {
        ...
        if (vs != null)
        {
            for (GridElement ge : this.gridElements)
            {
                if (ge.isInView(dc))
                {
                    if (ge.renderable instanceof GeographicText gt)
                    {
    ...
    protected class MetricScaleSupport
    {
        ...
        {
            if ((UTMZone != this.zone) || size < 1 || size > this.maxResolution)
                return;
    ...
    private class SquareSector
    {
        ...
        public boolean isInView(DrawContext dc)
        {
            ...
            if (!dc.getView().getFrustumInModelCoordinates().intersects(
                this.getExtent(dc.getGlobe(), dc.getVerticalExaggeration())) || (getSizeInPixels(dc) <= MIN_CELL_SIZE_PIXELS))
                return false;
        ...
        {
            if (!this.isPositionInside(this.nw) || !this.isPositionInside(this.ne) || !this.isPositionInside(this.sw) || !this.isPositionInside(this.se))
                return false;
            return true;
        }
    ...
    protected class SquareZone extends SquareSector
    {
        ...
        public void selectRenderables(DrawContext dc, Sector vs)
        {
            ...
            for (GridElement ge : this.gridElements)
            {
                ...
                {
                    if ((ge.type.equals(GridElement.TYPE_LINE_NORTH) && this.isNorthNeighborInView(dc)) || (ge.type.equals(GridElement.TYPE_LINE_EAST) && this.isEastNeighborInView(dc)))
                        continue;
    ...
    protected class SquareGrid extends SquareSector
    {
        ...
        public boolean isInView(DrawContext dc)
        {
            // Check apparent size
            if (!dc.getView().getFrustumInModelCoordinates().intersects(
                this.getExtent(dc.getGlobe(), dc.getVerticalExaggeration())) || (getSizeInPixels(dc) <= MIN_CELL_SIZE_PIXELS * 4))
                return false;
