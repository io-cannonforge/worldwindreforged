...
public class SurfacePolygon extends AbstractSurfaceShape implements Exportable
{
    ...
    protected InteriorVBOData buildInteriorVBOs(DrawContext dc, SurfaceTileDrawContext sdc)
    {
        ...
        // Positions are fp32 degree offsets from the reference; the MVP is composed in fp64 on the
        // CPU and cast to fp32 once at upload, preserving sub-metre precision without fp64 VBOs.
        ...
        // Vertex VBO — layout depends on hasTexCoords:
        //   no tex: tight float[x,y]                  stride 0   (8 bytes/vertex)
        //   tex:    interleaved float[x,y,s,t]        stride 16  (tex @ 8)
        gl.glBindBuffer(GL.GL_ARRAY_BUFFER, vboIds[0]);
        int stride = hasTexCoords ? 4 * Float.BYTES : 0;
        int texCoordByteOffset = hasTexCoords ? 2 * Float.BYTES : -1;
        ...
        return new InteriorVBOData(vboIds[0], vboIds[1], triangles.length,
            hasTexCoords, stride, GL.GL_FLOAT, texCoordByteOffset);
    }
