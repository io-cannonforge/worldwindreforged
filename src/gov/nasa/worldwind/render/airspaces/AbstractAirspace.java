    ...
    implements Airspace, OrderedRenderable, PreRenderable, Movable, Movable2, Draggable
{
    ...
    protected Collection<DetailLevel> detailLevels = new TreeSet<>();
    ...
    protected Map<LatLon, Double> elevationMap = new HashMap<>();
    ...
    protected void drawBatched(DrawContext dc)
    {
        ...
        else if (this.isEnableBatchPicking())
        {
            ...
            {
                AbstractAirspace airspace = (AbstractAirspace) nextItem;
                if (!airspace.isEnableBatchRendering() || !airspace.isEnableBatchPicking() || (airspace.pickLayer != this.pickLayer)) // batch pick only within a single layer
                    break;
    ...
    protected void beginRendering(DrawContext dc)
    {
        ...
        if (!dc.isPickingMode())
        {
            int attribMask = GL.GL_COLOR_BUFFER_BIT  // For color write mask, blending src and func, alpha func.
    ...
    protected void endRendering(DrawContext dc)
    {
        ...
        if (!dc.isPickingMode())
        {
            ...
            gl.glDisableClientState(GLPointerFunc.GL_NORMAL_ARRAY); // may have been enabled during rendering
        }
    ...
    protected void drawInterior(DrawContext dc)
    {
        ...
        if (!dc.isPickingMode())
        {
            if (attrs.isEnableLighting()) // Enable GL lighting state and set the current GL material state.
            {
                ...
                attrs.getInteriorMaterial().apply(gl, GL.GL_FRONT_AND_BACK, (float) attrs.getInteriorOpacity());
            }
    ...
    protected DetailLevel computeDetailLevel(DrawContext dc)
    {
        ...
        var iter = detailLevels.iterator();
        if (!iter.hasNext())
            return null;
