...
public class Polygon extends AbstractShape
{
    ...
    protected static class ShapeData extends AbstractShapeData implements Iterable<BoundaryInfo>
    {
        ...
        protected List<BoundaryInfo> boundaries = new ArrayList<>();
    ...
    {
        if ((this.getCurrent().coordBuffer == null) || (dc.getVerticalExaggeration() != this.getCurrent().getVerticalExaggeration()) || (this.mustApplyLighting(dc, null) && this.getCurrent().normalBuffer == null))
            return true;
	...
	protected boolean doMakeOrderedRenderable(DrawContext dc)
    {
        ...
        if (this.getCurrent().getExtent() == null || dc.isSmall(this.getExtent(), 1) || !this.intersectsFrustum(dc))
            return false;
...
//            dc.drawNormals(1000, this.boundarySet.coordBuffer, this.boundarySet.normalBuffer);
    ...
    protected Matrix computeRotationMatrix(Globe globe)
    {
        ...
        for (Position element : this.outerBoundary()) {
            Vec4 vert = globe.computePointFromPosition(element, 0);
