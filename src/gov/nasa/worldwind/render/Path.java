...
public class Path extends AbstractShape {
    ...
    protected boolean mustRegenerateGeometry(DrawContext dc) {
        if (this.getCurrentPathData() == null || this.getCurrentPathData().renderedPath == null || (this.getCurrentPathData().tessellatedPositions == null) || (dc.getVerticalExaggeration() != this.getCurrentPathData().getVerticalExaggeration())) {
            return true;
        }
    ...
    protected void computePositionCount() {
        ...
        if (this.positions != null) {
            for (Position ignored : this.positions) {
                ++this.numPositions;
            }
        }
    }
    ...
    protected FloatBuffer computePointsRelativeToTerrain(DrawContext dc, List<Position> positions,
        ...
        var colorIter = (pathData.tessellatedColors != null ? pathData.tessellatedColors.iterator() : null);
    ...
    protected boolean isSegmentVisible(DrawContext dc, Position posA, Position posB, Vec4 ptA, Vec4 ptB) {
        ...
        if (f.contains(ptA) || f.contains(ptB)) {
            return true;
        }
