 ...
 *
 * Changes:
 * - Added GPU fast path in computeLocations() using GpuTessellator.tessellateEllipse()
 *   to generate ellipse perimeter points via compute shader on GL 4.3+ hardware.
 *   Falls back to the original CPU loop when unavailable.
 */
...
public class SurfaceEllipse extends AbstractSurfaceShape
{
    ...
    protected List<LatLon> computeLocations(Globe globe, int intervals)
    {
        ...
        if (!Boolean.TRUE.equals(gpuTessellatorFailed.get()))
        {
            ...
            if (gl != null && gpuTessellator != null && gpuTessellator.isAvailable())
            {
