...
public class GDALUtils {
    ...
    protected static final CopyOnWriteArraySet<String> LOADED_LIBRARIES = new CopyOnWriteArraySet<>();
    protected static final CopyOnWriteArraySet<String> FAILED_LIBRARIES = new CopyOnWriteArraySet<>();
    ...
    protected static void replaceLibraryLoader() {
        try {
            ...
            // The LibraryLoader interface and setLibraryLoader method were removed in
            // newer GDAL Java bindings. Use reflection to stay compatible with both old
            // and new versions.
            boolean isKnownBuild = false;
            Method[] methods = gdalClass.getDeclaredMethods();
            for (Method m : methods) {
                if ("setLibraryLoader".equals(m.getName())) {
                    // Old GDAL version with LibraryLoader support - set it via reflection
                            ...
                            (proxy, method, args) -> {
                                if ("load".equals(method.getName()) && args != null && args.length == 1) {
                                    ...
                                    if (WWUtil.isEmpty(libName)) {
                                        String msg = Logging.getMessage("nullValue.LibraryIsNull");
                                    ...
                                    if (!FAILED_LIBRARIES.contains(libName)) {
                                        try {
                                            ...
                                            Logging.logger().finest(Logging.getMessage("generic.LibraryNotLoaded", libName, reason));
                                            FAILED_LIBRARIES.add(libName);
                                        }
                                    }
                                    String reason = Logging.getMessage("generic.LibraryNotFound", libName);
                                    throw new UnsatisfiedLinkError(Logging.getMessage("generic.LibraryNotLoaded", libName, reason));
                                }
                                return null;
                            }
                        );
    ...
    public static String getErrorMessage() {
        try {
            if (GDAL_IS_AVAILABLE.get()) {
                ...
                if (errno != gdalconstConstants.CE_None) {
    ...
    public static Dataset open(Object source, boolean isSilentMode)
        ...
        try {
            ...
            ds = gdal.Open(file.getAbsolutePath(), gdalconstConstants.GA_ReadOnly);
        } finally {
        ...
        for (int bandIdx = 0; bandIdx < bandCount; bandIdx++) {
            ...
            if (params.hasKey(AVKey.RASTER_BAND_MAX_PIXEL_VALUE)) {
                maxValue = (Double) params.getValue(AVKey.RASTER_BAND_MAX_PIXEL_VALUE);
                    ...
                    && colorInt != gdalconstConstants.GCI_AlphaBand && colorInt != gdalconstConstants.GCI_Undefined) {
            ...
            if (colorInt == gdalconstConstants.GCI_RedBand) {
                destBandIdx = 0;
            } else if (colorInt == gdalconstConstants.GCI_GreenBand) {
                destBandIdx = 1;
            } else if (colorInt == gdalconstConstants.GCI_BlueBand) {
                destBandIdx = 2;
            }
    ...
    protected static IntBuffer extractImageMask(AVList params) {
        ...
        try {
            ...
            if (o instanceof Dataset maskDS) {
...
//      GeoTransform[3] -= 0.5 * GeoTransform[5];
    ...
    public static AVList extractRasterParameters(Dataset ds, AVList params, boolean quickReadingMode)
        ...
        if (null != band) {
            ...
            if (dataType == gdalconstConstants.GDT_Int16 || dataType == gdalconstConstants.GDT_CInt16) {
                params.setValue(AVKey.PIXEL_FORMAT, AVKey.ELEVATION);
                params.setValue(AVKey.DATA_TYPE, AVKey.INT16);
            } else if (dataType == gdalconstConstants.GDT_Int32 || dataType == gdalconstConstants.GDT_CInt32) {
                params.setValue(AVKey.PIXEL_FORMAT, AVKey.ELEVATION);
                params.setValue(AVKey.DATA_TYPE, AVKey.INT32);
            } else if (dataType == gdalconstConstants.GDT_Float32 || dataType == gdalconstConstants.GDT_CFloat32) {
                params.setValue(AVKey.PIXEL_FORMAT, AVKey.ELEVATION);
                ...
                if (colorInt == gdalconstConstants.GCI_GrayIndex && bandCount < 3) {
                ...
                params.setValue(AVKey.PIXEL_FORMAT, AVKey.IMAGE);
                params.setValue(AVKey.DATA_TYPE, AVKey.INT16);
            } else if (dataType == gdalconstConstants.GDT_UInt32) {
                params.setValue(AVKey.IMAGE_COLOR_FORMAT,
                        ((bandCount >= 3) ? AVKey.COLOR : AVKey.GRAYSCALE));
                params.setValue(AVKey.PIXEL_FORMAT, AVKey.IMAGE);
                params.setValue(AVKey.DATA_TYPE, AVKey.INT32);
    ...
    protected static boolean setNoDataValue(Band band, Double nodata) {
        if (null != band && null != nodata) {
            try {
                ...
                return gdalconstConstants.CE_None == band.SetNoDataValue(nodata);
            } finally {
                gdal.PopErrorHandler();
            }
        }
    ...
    public static int[] extractBandOrder(Dataset ds, AVList params)
        ...
        if (params.hasKey(AVKey.BANDS_ORDER)) {
            ...
            for (int element : bandsOrder) {
                if (element < 0 || element >= bandsCount) {
                    String message = Logging.getMessage("generic.InvalidBandOrder", element, bandsCount);
                    Logging.logger().severe(message);
                    throw new IllegalArgumentException(message);
                }
            }
