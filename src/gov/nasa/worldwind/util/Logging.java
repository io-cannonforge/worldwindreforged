...
public class Logging
{
     ...
     * @return The logger.
     */
    // seaglassfoundry.com – Removed MESSAGE_BUNDLE_NAME from Logger.getLogger() call.
    // In Java 9+ the Logger resource-bundle lookup is sensitive to module/classloader
    // boundaries, causing MissingResourceException when .properties files are not on
    // the exact classloader the logging framework expects (e.g. Eclipse builds).
    // All message resolution already goes through Logging.getMessage() which calls
    // ResourceBundle.getBundle() directly, so the Logger never needs its own bundle.
