...
public class EGM2008
{
    ...
    public float getOffset(double lat, double lon) throws IOException
    {
        ...
        double baseLat = (latRow) * EGM2008.GRID_RESOLUTION;
        double baseLon = (lonCol) * EGM2008.GRID_RESOLUTION;
        float interpOffset = 0;
        for (int x = 0; x < 2; x++)
        {
            double cellLon = baseLon + (x) * EGM2008.GRID_RESOLUTION;
            for (int y = 0; y < 2; y++)
            {
                float cellOffset = latDataArray[y][lonCol + EGM2008.N_ROW_MARKERS / 2 + x];
                double cellLat = baseLat + (y) * EGM2008.GRID_RESOLUTION;
