	...
	public void selected(SelectEvent event)
    {
        ...
        else if (event.getEventAction().equals(SelectEvent.LEFT_CLICK))
        {
            ...
            if (event.getTopPickedObject().getParentLayer() == this.getControlPointLayer())
            {
                ...
                (this.getCurrentEvent().getMouseEvent().getModifiersEx() & InputEvent.SHIFT_DOWN_MASK) != 0)
            {
...
//            ((Airspace)shadowShape).setHighlightAttributes(shadowAttributes);
    ...
    protected void reshapePolygonAirspace(Position terrainPosition, ControlPointMarker controlPoint)
    {
        ...
        {
            if ((this.getCurrentEvent().getMouseEvent().getModifiersEx() & InputEvent.ALT_DOWN_MASK) != 0
            ...
            else if ((this.getCurrentEvent().getMouseEvent().getModifiersEx() & InputEvent.SHIFT_DOWN_MASK) != 0
                && this.isExtensionEnabled()
    ...
    protected void updatePolygonAirspaceControlPoints()
    {
        ...
        else
        {
            ...
            var markerIterator = markers.iterator();
            for (LatLon location : locations)
            {
                double altitude = this.getControlPointAltitude(location);
        ...
        var markerIterator = this.getControlPointLayer().getMarkers().iterator();
        for (int i = 0; i < locations.size(); i++)
        {
            markerIterator.next();
        }
    ...
    protected void updateRouteControlPoints()
    {
        ...
        var markerIterator = this.getControlPointLayer().getMarkers().iterator();
        for (int i = 0; i < locations.size(); i++) // skip over the locations to get to the width and rotation control points
        {
            markerIterator.next();
        }
    ...
    protected void updateTrackControlPoints()
    {
        ...
        ArrayList<Marker> controlPoints = new ArrayList<>();
        Iterable<Marker> markers = this.getControlPointLayer().getMarkers();
        var markerIterator = markers != null ? markers.iterator() : null;
