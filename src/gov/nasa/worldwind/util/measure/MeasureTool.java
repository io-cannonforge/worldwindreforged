...
public class MeasureTool extends AVListImpl implements Disposable {
    ...
    protected static class CustomRenderableLayer extends RenderableLayer implements PreRenderable, Renderable {
        ...
        public void render(DrawContext dc) {
            if ((dc.isPickingMode() && !this.isPickEnabled()) || !this.isEnabled()) {
                return;
            }
    ...
    protected void swapCornerControls(String control, Position position) {
        ...
        if (control.equals(newControl) || control.length() != 2 || newControl.length() != 2) {
            return;
        }
    ...
    protected LatLon computeQuadEdgeMidpointLocation(String control, Globe globe, Angle heading, LatLon center,
        ...
        return switch (control) {
            case NORTH -> LatLon.interpolate(this.pathType, 0.5, nw, ne);
            case EAST -> LatLon.interpolate(this.pathType, 0.5, ne, se);
            case SOUTH -> LatLon.interpolate(this.pathType, 0.5, sw, se);
            case WEST -> LatLon.interpolate(this.pathType, 0.5, sw, nw);
            default -> null;
        };
    }

    protected Angle computeControlPointAzimuth(String control, double width, double height) {
        Angle azimuth = switch (control) {
            case NORTH, NORTH_LEADER -> Angle.ZERO;
            case EAST -> Angle.POS90;
            case SOUTH -> Angle.POS180;
            case WEST -> Angle.fromDegrees(270);
            case NORTHEAST -> Angle.fromRadians(Math.atan2(width, height));
            case SOUTHEAST -> Angle.fromRadians(Math.atan2(width, -height));
            case SOUTHWEST -> Angle.fromRadians(Math.atan2(-width, -height));
            case NORTHWEST -> Angle.fromRadians(Math.atan2(-width, height));
            default -> null;
        };
    ...
    protected Angle computeControlPointAzimuthInShapeCoordinates(String control, Angle azimuth) {
        return switch (control) {
            case NORTHEAST -> azimuth.subtract(this.shapeOrientation);
            case SOUTHEAST -> this.shapeOrientation.addDegrees(180).subtract(azimuth);
            case SOUTHWEST -> azimuth.subtract(this.shapeOrientation.addDegrees(180));
            case NORTHWEST -> this.shapeOrientation.subtract(azimuth);
            default -> null;
        };
    }
