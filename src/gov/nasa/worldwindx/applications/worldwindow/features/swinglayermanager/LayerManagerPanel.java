...
//        layerTree.setToolTipText(TOOL_TIP);
    ...
    protected void updateGroupSelections()
    {
        ...
        Enumeration<TreeNode> iter = ((LayerTreeModel) this.layerTree.getModel()).getRootNode().depthFirstEnumeration();
    ...
    {
        // Modified by seaglassfoundry.com - added generic type parameters
        Enumeration<TreeNode> iter = group.breadthFirstEnumeration();
        while (iter.hasMoreElements())
        {
    ...
    {
        if (this.insertAfterPreviousSibling(treeNode, layerList) || this.insertBeforeSubsequentSibling(treeNode, layerList))
            return;
